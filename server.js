...
const FALLBACK_MODELS = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-2.5-flash-lite', 'gemini-flash-lite-latest'];
...
app.get('/api/kqxs', async (req, res) => {
  ...
  if (!date || !province) return res.status(400).send('Thiếu tham số.');
  let targetUrl;

  if (province === 'mien-bac') {
    targetUrl = `https://www.minhngoc.net.vn/ket-qua-xo-so/mien-bac/${date}.html`;
  } else {
    targetUrl = `https://www.minhngoc.net.vn/ket-qua-xo-so/mien-nam/${province}/${date}.html`;
  }
...
`;
    ...
    res.json({ aiText: cleanText });
  } catch (e) {
    console.error('Lỗi cuối cùng:', e);
    res.status(500).json({ error: 'Không thể phân tích AI vào lúc này.' });
  }
});
