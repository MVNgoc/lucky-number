...
async function fetchDaiList() {
  ...
  try {
    ...
    console.error('❌ Không thể lấy danh sách đài:', err.message);
...
async function fetchScheduleSlugs(weekday) {
  ...
  try {
    ...
    console.error(`❌ Không lấy được lịch quay thứ ${weekday}:`, err.message);
...
async function fetchDaiListForWeekday(weekday) {
  if (weekday === null) return { groups: await fetchDaiList(), weekday: null };
  ...
  if (filtered.length === 0) return { groups, weekday };

  return { groups: filtered, weekday };
}
...
async function findProvinceByName(name) {
  if (typeof name !== 'string' || !name.trim()) return null;

  const groups = await fetchDaiList();
  const all = groups.flatMap(g => g.provinces);
