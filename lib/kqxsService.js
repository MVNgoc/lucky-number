...
// Kết quả một kỳ quay đã công bố thì không bao giờ đổi -> cache dài. Riêng
// ngày hôm nay có thể chưa tới giờ công bố (các đài quay từ chiều tối) nên
// cache ngắn, không thì người dùng bị kẹt với "chưa có kết quả" cả tiếng.
const PAST_DAY_TTL_MS = 24 * 60 * 60 * 1000;
const TODAY_TTL_MS = 60 * 1000;
// Mỗi trang ~160KB nên phải chặn số entry, không thì cache ăn hết RAM
...
function readCache(key) {
  ...
  // Chạm vào là coi như mới dùng, đẩy xuống cuối để LRU đúng thứ tự
  cache.delete(key);
  cache.set(key, entry);
  return entry.html;
}
...
function writeCache(key, html, ttlMs) {
  ...
  while (cache.size > MAX_CACHE_ENTRIES) {
    // Map giữ thứ tự chèn -> phần tử đầu là ít dùng nhất
